...
function TeacherProfileSetting() {
            ...
            if (file) {
                ...
                img.onload = function () {
                    ...
                    if (width !== 300 || height !== 300) {
                        resolve(false); // dimensions are not 300x300
                    } else {
                        resolve(true); // dimensions are 300x300
                    }
                };
                img.src = URL.createObjectURL(file);
            } else {
                resolve(false); // no file provided
            }
        });
    }
    ...
    const SubmitForm = async () => {
        ...
        const isImageValid = await chk_img_size(teacherData.profile_image);
        if (isImageValid) {
            try {
